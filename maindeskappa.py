...
def train(opt):
    ...
    optim = t.optim.SGD(model.parameters(), lr=0.00001, weight_decay=5e-5,momentum=0.9)
...
def val(model, val_loader):
    ...
    for ii ,(imgs, data, label) in enumerate(val_loader):
        print(ii)
        imgs = imgs.cuda()
        data = data.cuda()
        label = label.cuda()

        pre_ = model(imgs, data)
...
@t.no_grad()
def test():
    #model = t.load('zoom2_34.pth')
    #model = model.cuda()
    model = t.load('inres_80.pth')
    con_matx = meter.ConfusionMeter(5)
    test_data = Eye_img(test=True)
    num = len(test_data)//128
    test_loader = DataLoader(test_data, 128, False)
    for ii ,(imgs, data, label) in enumerate(test_loader):
        # print('eopch[{}/{}]'.format(ii, num))
        #imgs = imgs.cuda()
        data = data.cuda()
        label = label.cuda()
        ...
        for i in range(5):
            cm_sum += cm_value[i][i]
        acc = 100.*(cm_sum)/cm_value.sum()
        print('eopch[{}/{}] | acc:{:.02f} | kappa:{:.02f}'.format(ii, num, acc, kappa_*100))
